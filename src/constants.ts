...
import { TileConfig, ThemeConfig, ThemeId, MathQuestion } from './types';

// 30 Steps Layout (Snake Pattern)
// Designed for 16:9 Landscape
export const BOARD_PATH_COORDS: {x: number, y: number}[] = [];

// Manually define a nice 30-step snake path
// 3 Rows of roughly 10 steps, or 5 rows of 6. 
// Let's do 5 rows of 6 for better vertical spacing.
...
for (let row = 0; row < ROWS; row++) {
  const y = 85 - (row * 16); // Y positions: 85, 69, 53, 37, 21
  const isEvenRow = row % 2 === 0;
  
  for (let col = 0; col < COLS; col++) {
    const x = isEvenRow 
      ? 15 + (col * 14)  // Left to Right
      : 85 - (col * 14); // Right to Left
    
    BOARD_PATH_COORDS.push({ x, y });
  }
}

// NOTE: Tile Types are randomized in GameBoard.tsx
...
export const THEMES: Record<ThemeId, ThemeConfig> = {
  ...
  WINTER: {
    id: 'WINTER',
    name: 'เมืองหิมะ',
    bgClass: 'theme-ocean', // Reuse ocean for cool tones
    primaryColor: '#3b82f6',
    secondaryColor: '#ffffff',
  ...
  FARM: {
    id: 'FARM',
    name: 'ฟาร์มสุขสันต์',
    bgClass: 'theme-jungle', // Reuse jungle for greens
    primaryColor: '#eab308',
